...
<main class="container vh-100 mt-5">
  <header class="row">
    ...
    <section>
      <form method="POST" action="{{ route('edit',['id'=> $artworks->id]) }}" class="row my-4 form" enctype="multipart/form-data" onsubmit="return validation()">
        @method('PATCH')
        @csrf
        {{--  primera columna --}}
        ...
        <div class="col-10 pe-0">
          ...
          <input type="text" class="select" name="title" value="{{old('title') ?? $artworks->title}}"> 
          <input type="text" class="select">
          <select name="artist" id="#" class="select" name="author" value="{{old('author') ?? $artworks->author}}" checked>
            <option value="1">Adam Birkett</option>
            <option value="2">Jon Doe</option>
            <option value="3">Maria la Portuguesa</option>
          </select>
          <input type="text" class="ps-2 select" name="age" value="{{old('age') ?? $artworks->age}}" >
          <input type="text" class="ps-2 select" name="stockNumber" value="{{old('stockNumber') ?? $artworks->stockNumber}}">
          ...
          <div class="mt-5 group-input">
            <input type="number" class="dimensions-input" name="size" value="{{old('size') ?? $artworks->size}}">
        ...
        <div class="mt-3 group-btn">
          <button class="text-white btn-save">Save</button>
